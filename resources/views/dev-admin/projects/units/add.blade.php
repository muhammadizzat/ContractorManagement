                ...
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        ...
                        <div class="pr-3 text-right">
                            <a href="{{ route('dev-admin.projects.units.index', $proj_id) }}"
                                class="btn btn-sm btn-primary">{{ __('Cancel') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form role="form" method="post" action="{{route('dev-admin.projects.units.add.post', $proj_id)}}"
                        ...
                        <div class="pl-lg-4">
                            ...
                            <div class="form-group{{ $errors->has('unit_no') ? ' has-danger' : '' }}">
                                <input type="text" name="unit_no" id="input-unit-no"
                                    class="form-control form-control-alternative{{ $errors->has('unit_no') ? ' is-invalid' : '' }}"
                                    placeholder="{{ __('Unit No') }}" value="{{ old('unit_no') }}" required autofocus>
                            ...
                            <div class="form-group{{ $errors->has('unit_type_id') ? ' has-danger' : '' }}">
                                <select name="unit_type_id" id="unit_type_id"
                                    ...
                                    placeholder="{{ __('Unit Type ID') }}" title="Choose Unit Type"
                        ...
                        <div class="pl-lg-4">
                            ...
                            <div class="form-group{{ $errors->has('owner_name') ? ' has-danger' : '' }}">
                                <input type="text" name="owner_name" id="input-owner-name"
                                    class="form-control form-control-alternative{{ $errors->has('owner_name') ? ' is-invalid' : '' }}"
                                    placeholder="{{ __('Owner Name') }}" value="{{ old('owner_name') }}">
                            </div>
                            ...
                            <div class="form-group{{ $errors->has('owner_contact_no') ? ' has-danger' : '' }}">
                                <input type="text" name="owner_contact_no" id="input-owner-contact-no"
                                    class="form-control form-control-alternative{{ $errors->has('owner_contact_no') ? ' is-invalid' : '' }}"
                                    maxlength="15" placeholder="{{ __('Owner Contact No') }}"
                                    value="{{ old('owner_contact_no') }}">
                            ...
                            <div class="form-group{{ $errors->has('owner_email') ? ' has-danger' : '' }}">
                                <input type="email" name="owner_email" id="input-owner-email"
                            ...
                            <div class="text-center">
                                <button onclick="return confirm('Are you sure you want to add this unit?')"
