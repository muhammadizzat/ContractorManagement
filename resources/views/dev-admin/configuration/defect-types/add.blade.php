            ...
            <div class="card bg-secondary shadow">
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        <div class="col">
                            <h3 class="mb-0">{{ __('Add Defect Type') }}</h3>
                        </div>
                        <div class="pr-3 text-right">
                            <a href="{{ route('dev-admin.configuration.defect-types.index') }}"
                                class="btn btn-sm btn-primary">{{ __('Cancel') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form class="prevent-multiple-submits" method="post"
                        action="{{ route('dev-admin.configuration.defect-types.add.post') }}" autocomplete="off" novalidate>
                        @csrf
                        ...
                        <div class="pl-lg-4">
                            ...
                            <div class="form-group{{ $errors->has('details') ? ' has-danger' : '' }}">
                                <input type="text" name="details" id="input-details"
                                    ...
                                    placeholder="{{ __('Details') }}" value="{{ old('details') }}">
                            ...
                            <div class="text-center">
                                <button type="submit" class="btn btn-success btn-submit mt-4">{{ __('Add') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
