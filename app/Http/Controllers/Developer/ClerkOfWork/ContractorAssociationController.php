...
class ContractorAssociationController extends Controller
{
    ...
    {
        return view('dev-cow.associations.index');
    }

    public function getDataTableContractorAssociation(Request $request)
    {
        if ($request->ajax()) {
            $developer_id = request()->user()->clerk_of_work->developer_id;
            ...
            return DataTables::of($developer_contractor_association)
                ...
                ->addColumn('editAssociationUrl', function ($developer_contractor_association) {
                    return route('dev-cow.associations.edit', ['id' => $developer_contractor_association['id']]);
                })
                ->addColumn('deleteAssociationUrl', function ($developer_contractor_association) {
                    return route('dev-cow.associations.delete', ['id' => $developer_contractor_association['id']]);
                })
                ->make(true);
        }
    }
    ...
    public function addContractorAssociation(DeveloperContractorAssociation $developer_contractor_association)
    {
        $developer_id = request()->user()->clerk_of_work->developer_id;
        $defect_type_list = DefectType::where('developer_id',$developer_id)->orWhere('developer_id', null)->select('id','title')->get();

        return view('dev-cow.associations.add', ['developer_id' =>$developer_id, 'defect_type_list' => $defect_type_list]);
    }
    ...
    public function postAddContractorAssociation(Request $request)
    {
        ...
        return redirect()->route('dev-cow.associations.index')->withStatus(__('Contractor Scope of Work successfully added.'));
    }
    ...
    public function editContractorAssociation(DeveloperContractorAssociation $developer_contractor_association, Request $request, $id)
    {
        ...
        return view('dev-cow.associations.edit', ['developer_contractor_association' => $developer_contractor_association, 'id' => $id, 'developer_id' =>$developer_id, 'defect_type_list' => $defect_type_list]);
    }
    ...
    public function postEditContractorAssociation(Request $request)
    {
        ...
        $dca->defect_types()->sync($request['defect_type_ids']);
        return redirect()->route('dev-cow.associations.index')->with('status', 'Contractor Assocation has been edited.');
    }
    public function deleteContractorAssociation(Request $request)
    {
        ...
        if ($defect_count != 0) {
            return redirect()->route('dev-cow.associations.index')->with('error', 'This contractor is being assigned to defect(s).');
        } else {
            $dca->defect_types()->sync([]);
            $dca->delete();
            return redirect()->route('dev-cow.associations.index')->with('status', 'Contractor Scope of Work has been successfully deleted.');
        };
    }
}
