...
class CaseController extends Controller
{
    ...
    public function index($proj_id)
    {
        ...
        return view('dev-cow.projects.cases.index', ['cases' => $cases, 'proj_id' => $proj_id]);
    }
    ...
    public function getDataTableCases(Request $request, $proj_id)
    {
        ...
        return DataTables::of($cases)
            ...
            ->addColumn('viewUrl', function ($row) {
                return route('dev-cow.projects.cases.view', ['proj_id' => $row->project_id, 'id' => $row->id]);
            })
            ->addColumn('deleteUrl', function ($row) {
                return route('dev-cow.projects.cases.delete', ['proj_id' => $row->project_id, 'id' => $row->id]);
            })
            ->make(true);
    }
    ...
    public function addCase($proj_id)
    {
        ...
        return view('dev-cow.projects.cases.add', ['proj_id' => $proj_id, 'units' => $units]);
    }
    ...
    public function postAddCase($proj_id)
    {
        ...
        return redirect()->route('dev-cow.projects.cases.view', ['proj_id' => $proj_id, 'id' => $case->id])->with('status', 'Case is successfully added.');
    }
    ...
    public function deleteCase(Request $request, $proj_id)
    {
        ...
        if ($defects == 0) {
            ...
            return redirect()->route('dev-cow.projects.cases.index', ['project_id' => $proj_id])->with('status', 'Case is successfully deleted.');
        } else {
            return redirect()->route('dev-cow.projects.cases.index', ['project_id' => $proj_id])->with('error', 'Case cannot be deleted due to defects already created under this case');
        }
    }
    ...
    public function viewCase($proj_id, $id)
    {
        ...
        return view('dev-cow.projects.cases.view', ['developer_id' => $developer_id, 'proj_id' => $proj_id, 'case' => $case]);
    }
